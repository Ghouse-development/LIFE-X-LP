  ...
  return (
    <Section id="process" variant="light" spacing="2xl">
      ...
      <div className="relative">
        ...
        <div className="hidden md:block absolute top-12 left-0 right-0 h-0.5 bg-[var(--accent)]/20" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {data.map((step, index) => (
            ...
            >
              {/* Timeline node */}
              <div className="hidden md:flex absolute top-12 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-6 h-6 bg-[var(--accent)] rounded-full border-4 border-[var(--surface-alt)] z-10" />
              ...
              <div className="rounded-2xl border border-black/5 bg-white p-6 shadow-[0_2px_20px_rgba(0,0,0,0.05)] mt-20 md:mt-16 flex flex-col h-full">
                {/* Step Number Badge */}
                <div className="inline-flex items-center justify-center w-12 h-12 bg-[var(--accent)] rounded-full mb-4">
                  <span className="font-serif text-xl font-bold text-white">{step.step}</span>
                </div>

                {/* Title */}
                <h3 className="font-serif text-xl font-bold text-[var(--ink-strong)] mb-2">{step.title}</h3>
                ...
                <div className="border-t border-black/5 pt-4 space-y-2 text-xs">
                  <div className="flex items-start gap-2">
                    ...
                    <span className="text-[var(--accent)] font-medium">{step.duration}</span>
                  </div>
                  {step.deliverables && (
                    <div className="flex items-start gap-2">
              ...
              {index < data.length - 1 && (
                <div className="hidden md:block absolute top-12 right-0 transform translate-x-1/2 -translate-y-1/2">
                  <ArrowRight className="text-[var(--accent)] w-5 h-5" />
                </div>
              )}
            </motion.div>
          ))}
